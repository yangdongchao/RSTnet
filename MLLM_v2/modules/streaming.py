...
State = tp.TypeVar("State", bound=Resetable) # 定义一个可以reset的状态变量
...
class StreamingModule(abc.ABC, nn.Module, tp.Generic[State]):
    ...
    @property
    def is_streaming(self): # 判断是否在streaming状态
        return self._streaming_state is not None

    def set_streaming_propagate(self, streaming_propagate: bool): # 设置streaming_propagate 状态,是否传递给子类?
    ...
    def _apply_named_streaming(self, fn: tp.Any):
        def _handle_module(prefix: str, module: nn.Module, recurse: bool = True):
            ...
            if isinstance(module, StreamingModule):
                if module._streaming_propagate: # if _streaming_propagate is true, we can apply the function
                    fn(prefix, module)
                else:
                    propagate = False
            if not recurse:
    ...
    def _start_streaming(self, batch_size: int): # prepare the streaming state for all modules
        def _start_streaming(name: str, module: StreamingModule):
            module._streaming_state = module._init_streaming_state(batch_size) # init the streaming state
    ...
    # the function _init_streaming_state will be re-write by child class
...
# 在流式处理系统中，有时不需要维护复杂的状态。这种情况下使用 _NullState 可以简化状态管理，因为它不需要实际存储任何数据。
