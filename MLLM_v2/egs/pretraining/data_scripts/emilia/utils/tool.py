...
def export_to_mp3(audio, asr_result, folder_path, file_name):
    """Export segmented audio to MP3 files and return dictionaries mapping utterances to paths and JSON metadata."""
    ...
    def process_segment(idx, segment):
        ...
        out_path = os.path.join(audio_folder_path, out_file)
        write_mp3(out_path, sr, split_audio)

        utt2path[f"{file_name}_{idx}"] = out_path

        metadata_path = os.path.join(folder_path, "metadata")
        os.makedirs(metadata_path, exist_ok=True)
        metadata_file = os.path.join(metadata_path, f"{file_name}_{idx}.json")
        with open(metadata_file, "w") as f:
            json.dump(segment, f)
        
        utt2json[f"{file_name}_{idx}"] = metadata_file
