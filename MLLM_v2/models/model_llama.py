'''The pre-training model code, written by Dongchao. The codebase is from Kyutai
The code aims to (1) support different LLM codebase, e.g. LLAMA
                 (2) add local transformer for semantic and acoustic modeling
for the global transformer, we use the pre-trained LLM. Furthermore, we fix the LLM and add lora for it.
for the local transformer, we follow Moshi's setting
''' 
...
def CrossEntropyAndAccuracy(logits, y, masks, loss_weights, ignore_ids=None):
    ...
    for idx, w in enumerate(loss_weights):
        ...
        tmp_num_all_tokens = tmp_y.ne(ignore_ids[idx]).int().sum()
        tmp_acc_tk = torch.logical_and(tmp_pred.eq(tmp_y), tmp_y.ne(ignore_ids[idx])).int().sum()
        num_all_tokens += tmp_num_all_tokens
        acc_tk += tmp_acc_tk
        loss += (tmp_loss.sum()/tmp_num_all_tokens)*loss_weights[idx]
    acc_all = acc_tk/num_all_tokens
    metrics = {'acc_all': acc_all, 'loss': loss.clone().detach()}
    return loss, metrics
...
class MLMModel(StreamingContainer):
    """Transformer-based multi-modal language model on multiple streams of codes.
       The global transformer, we use LLAMA 7B. But we use Lora and fix it.
       For the local transformer, we follow UniAudio and Moshi
    Args:
