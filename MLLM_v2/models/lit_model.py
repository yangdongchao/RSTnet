...
class CausalSelfAttention(nn.Module):
    ...
    def scaled_dot_product_attention(
        ...
        if self.config.attention_logit_softcapping is not None: #attention_logit_softcapping==none
            scale = 1.0 / math.sqrt(self.config.attention_scores_scalar or self.config.head_size)
            scores = q @ k.mT * scale
            scores = (
...
class RingKVCache:
    ...
    def complete(self, k: torch.Tensor, v: torch.Tensor) -> KVCacheResult:
        ...
        self.cache[0].index_copy_(2, indexes, k) # dim=2 表示沿着第三个维度（通常是注意力头数或其他维度）
        self.cache[1].index_copy_(2, indexes, v)
        self.end_offset.add_(T)

        keys = self.cache[0] # get the added key
        values = self.cache[1] # get the added values

        indexes = torch.arange(
            self.capacity, device=self.end_offset.device, dtype=torch.long
        )
        invalid = indexes >= self.end_offset # 标记超出 end_offset 的索引（这些位置是无效的）。

        end_index = self.end_offset % self.capacity
        delta = indexes - end_index # 在 end_index之前的都会小于0
