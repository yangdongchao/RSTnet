# Author: Jinchuan Tian & Dongchao Yang
...
from tools.tokenizer.layer_codec.one_tokenizer import AudioOneTokenizer
from tools.tokenizer.Text2ID.text_tokenizer import Text2IDTokenizer
def get_parser():
    ...
    parser.add_argument("--tokenizer", type=str, choices=['audio', 'audio_one', 'clipbpe', 'semantic', 'text'], help="what tokenizer to use")
...
def main(args):
    ...
    if args.tokenizer in ['audio', 'audio_one', 'semantic', 'encodec']:
    ...
    # GPU tokenizers 
    print('args.tokenizer ', args.tokenizer)
    assert args.tokenizer == 'audio_one'
    if args.tokenizer == 'audio_one':
        tokenizer = AudioOneTokenizer(device=device)
    elif args.tokenizer == 'text':
        tokenizer = Text2IDTokenizer()
    else:
            ...
            try:
                ...
                for info in tar_o: # get the speech info
                    ...
                    if info.name.split(".")[-1] == args.tar_key_word:
                        ...
                        tar_new_name = tar_path.replace('/blob/v-dongyang/AudioBox/','')
