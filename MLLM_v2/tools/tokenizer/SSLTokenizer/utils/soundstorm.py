...
class SoundStorm(nn.Module):
    def __init__(
        ...
        use_phone_prefix = (
            cfg.use_phone_prefix
            if cfg is not None and hasattr(cfg, "use_phone_prefix")
            else use_phone_prefix
        )
        cond_code_layers = (
        ...
        self.use_phone_prefix = use_phone_prefix
        ...
        if not self.use_llama_style:
            ...
            if not self.use_phone_cond:
                ...
                if not self.use_phone_prefix:
                    ...
                    from models.tts.soundstorm.llama_nar_prefix import DiffLlamaPrefix
    ...
    def loss_t(self, x0, x_mask, t, cond=None, phone_embedding=None, phone_mask=None):
        ...
        if self.use_phone_cond and phone_embedding != None:
            embeds = self.diff_estimator(xt, new_t, cond, x_mask, phone_embedding=phone_embedding, phone_mask=phone_mask)  # (B, T, hidden_size)
        else:
    ...
    def compute_loss(self, x0, x_mask, cond=None, phone_embedding=None, phone_mask=None):
        # x0: (B, T, num_quantizer)
        # x_mask: (B, T) mask is 0 for padding
        t = torch.rand(x0.shape[0], device=x0.device, requires_grad=False)
        t = torch.clamp(t, 1e-5, 1.0)
        return self.loss_t(x0, x_mask, t, cond, phone_embedding, phone_mask)
    ...
    def reverse_diffusion(
        ...
        if lang_id != None and self.use_lang_emb:
            lang_embedding = self.lang_emb(lang_id)
            phone_embedding += lang_embedding
        ...
        for mask_layer in range(gt_layer, max_layer):
            ...
            for i in range(steps):
                ...
                if phone_embedding != None and self.use_phone_cond:
                    embeds = self.diff_estimator(xt_input, t, temp_cond, xt_mask, phone_embedding=phone_embedding, phone_mask=phone_mask)
                else:
                ...
                if phone_embedding != None and self.use_phone_cond:
                    mask_embeds = self.diff_estimator(
                        cur, t, temp_cond[:, prompt_len:, :], x_mask, phone_embedding=phone_embedding, phone_mask=phone_mask
                    )
                else:
                    mask_embeds = self.diff_estimator(
    ...
    def forward(self, x0, x_mask, cond=None, cond_code=None, phone_id=None, phone_mask=None, lang_id=None):
        # x0: (B, T, num_quantizer)
        # x_mask: (B, T) mask is 0 for padding
        ...
        if lang_id != None and self.use_lang_emb:
            lang_embedding = self.lang_emb(lang_id).unsqueeze(1) * phone_mask.unsqueeze(-1)
            phone_embedding += lang_embedding

        logits, mask_layer, final_mask, x0, prompt_len, mask_prob = self.compute_loss(
            x0, x_mask, cond, phone_embedding, phone_mask=phone_mask
        )
        return logits, mask_layer, final_mask, x0, prompt_len, mask_prob
