...
class MultiFrequencyDiscriminator(nn.Module):
    def __init__(self, config: DictConfig):
        ...
        for bw in config.target_bandwidths:
            self.discs[f'disc_bw{bw}'.replace('.', '_')] = InnerMultiFrequencyDiscriminator(config)

    def forward(self, y, y_hat, bw):
        return self.discs[f'disc_bw{bw}'.replace('.', '_')](y, y_hat)
    

class InnerMultiFrequencyDiscriminator(nn.Module):
    def __init__(self, config: DictConfig):
        super().__init__()
