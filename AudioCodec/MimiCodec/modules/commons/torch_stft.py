...
class TorchSTFT(torch.nn.Module):
    ...
    def transform(self, x):
        with autocast(enabled=False):
            x = x.float()
            x_stft = torch.stft(x, self.fft_size, self.hop_size, self.win_size, self.window.type_as(x), normalized=self.normalized,return_complex=True) # return_complex=True
        x_stft = torch.view_as_real(x_stft) # for new version torch
        real = x_stft[..., 0]
        imag = x_stft[..., 1]
