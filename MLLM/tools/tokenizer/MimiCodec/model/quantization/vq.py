...
from tools.tokenizer.MimiCodec.model.quantization.base import BaseQuantizer, QuantizedResult
from tools.tokenizer.MimiCodec.model.quantization.core_vq import ResidualVectorQuantization
...
class ResidualVectorQuantizer(BaseQuantizer):
    ...
    def __init__(
        ...
        self.no_quantization_rate = no_quantization_rate # does not need in our code
    ...
    def forward(self, x: torch.Tensor, frame_rate: int):
        ...
        if self.q_dropout and self.n_q > 1:
            # apply the quantizer dropout
            n_q = torch.randint(1, self.n_q+1) # 1 ~ n_q
        else:
            n_q = self.n_q
...
class SplitResidualVectorQuantizer(BaseQuantizer):
    ...
    def forward(self, x: torch.Tensor, frame_rate: int, semantic_features: torch.Tensor=None):
        """
        Args:
            x (torch.Tensor): Input tensor of shape [B, C, T] with `C` number of channels.
            frame_rate (int): frame rate of the input (e.g `T = frame_rate * duration`), used to compute
        ...
        if semantic_features is not None:
            sim_loss = self.cosine_similarity_loss(semantic_result.x.transpose(1,2), semantic_features)
        else:
            sim_loss = 0.0
